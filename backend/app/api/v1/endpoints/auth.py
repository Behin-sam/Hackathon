...
def _safe_uuid(value: str | None) -> uuid.UUID | None:
    """Parse a JWT `sub` claim into a UUID, returning None if malformed
    rather than letting the raw string reach the database driver (which
...
async def confirm_password_reset(
    ...
    user_id = _safe_uuid(decoded.get("sub"))
    if user_id is None:
        raise UnauthorizedError("Invalid or expired password reset token")
